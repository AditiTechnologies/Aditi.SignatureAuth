...
Function PatchFile()
{
    ...
    $content = Get-Content $file | foreach-object {
         $_ -replace '^\[assembly: AssemblyVersion\("1.0.0.0"\)\]', "[assembly: AssemblyVersion(`"$assemblyVersion`")]" `
                ...
                -replace '^\[assembly: AssemblyDescription\("([^"]*)"\)]',"[assembly: AssemblyDescription(`"`$1 Ver-$assemblyInformationalVersion`")]" 
...
if ($action -eq "patch")
{
    gci *.csproj  -Recurse |
        ...
        foreach-object {
            ...
            $projSemVer = (Get-Semver -path $($_.Directory) -branch $branch -rev $rev -buildCounter $buildCounter)
